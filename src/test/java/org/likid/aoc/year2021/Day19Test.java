...
class Day19Test {
    ...
    @Test
    void should_day19() throws IOException {
        System.out.println("day 19");
        List<String> inputs = Util.readFileAsString("classpath:year2021/day19/example");
        var objects = mapScanners(inputs).stream().map(ScannerVariations::new).toList();
        ScannerVariations[] scannerVariations = new ScannerVariations[objects.size()];
